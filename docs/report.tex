...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Inferenčné pravidlá} --- pomocné, hlavné
        ...
        odporúčanie (\texttt{backend/program.pl}, sekcie 2A--2G).
  \item \textbf{HTTP API} --- tenká JSON vrstva \texttt{backend/api.pl}
        nad \texttt{program.pl}; každý handler pozbiera riešenia
        cez \texttt{findall/setof} a~pošle \texttt{reply\_json\_dict};
        beží na porte 4000 cez \texttt{library(http/thread\_httpd)}.
  \item \textbf{Webová aplikácia} --- interaktívny playground
        v~Next.js 15 (\texttt{web/}); všetka logika zostáva
        v~Prologu, frontend robí len proxy dopytov a~vizualizáciu.
\end{enumerate}
...
webový \emph{playground}. Architektúra sa skladá z~dvoch kontajnerov
riadených Docker Compose:

\paragraph{Backend (\texttt{backend/api.pl})} je tenká HTTP JSON
vrstva nad nezmeneným \texttt{program.pl}. Používa štandardnú
knižnicu \texttt{library(http/thread\_httpd)} a~vystavuje 13
endpointov --- napr.\ \texttt{/api/recommend}, \texttt{/api/hybrid},
\texttt{/api/cross-domain} či \texttt{/api/movie-night}.
Každý handler žiadnu logiku neduplikuje: iba pozbiera riešenia
cez \texttt{findall/setof} a~prevedie Prolog termy na JSON
pomocou \texttt{reply\_json\_dict}. Dôvody odporúčaní
(štruktúrované Prolog termy) prevádza predikát
\texttt{dovod\_json/2} na JSON objekty so štandardizovaným
poľom \texttt{druh}.

\paragraph{Frontend (\texttt{web/})} je aplikácia v~Next.js~15
(App Router) s~Tailwind CSS. Stránka je renderovaná dynamicky
(\texttt{force-dynamic}) --- každý dopyt ide na živý Prolog engine.
Playground obsahuje deväť interaktívnych sekcií:
content-based odporúčania s~vysvetleniami, hybridné skóre,
collaborative filtering, cross-domain odporúčania, reverzný
filter fanúšikov, graf vzťahov, constraint filmový večer
a~štatistiky.

Celý systém sa spustí jedným príkazom:
\begin{lstlisting}[language=bash, numbers=none]
docker compose up --build
\end{lstlisting}
a~je dostupný na \texttt{http://localhost:3000}.
...
Člen 2 & Hlavné inferenčné pravidlo \texttt{odporucam/2},
         ...
         rozmerov & \texttt{backend/program.pl} (§2.A--B)\\
Člen 3 & Pokročilá inferencia: skórovanie, top, vysvetlenie,
         ...
         \texttt{backend/program.pl} (§2.C--G)\\
Člen 4 & Demonštračné príklady (10~scenárov), PlUnit regresné
         testy, \textbf{HTTP API} (\texttt{api.pl}),
         \textbf{webová aplikácia} (Next.js) &
         \texttt{backend/examples.pl}, \texttt{backend/tests.pl},
         \texttt{backend/api.pl}, \texttt{web/},
         \texttt{README.md}\\
Člen 5 & Elaborát, prezentácia, koordinácia tímu,
         Docker Compose a~nasadenie &
         \texttt{docs/report.tex},
